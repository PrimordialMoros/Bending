...
public class BendingCommand<T extends Audience> implements ElementHandler {
  ...
  BendingCommand(BendingPlugin plugin, CommandManager<T> manager, Class<? extends T> playerClass) {
    this.plugin = plugin;
  ...
  private void onInfo(T sender, AbilityDescription ability) {
    Component description = plugin.translationManager().translate(ability.descriptionKey()).orElse(null);
    Component instructions = plugin.translationManager().translate(ability.instructionsKey()).orElse(null);
