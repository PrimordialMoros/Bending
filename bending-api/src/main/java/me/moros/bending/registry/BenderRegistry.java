...
public final class BenderRegistry implements Registry<User> {
  ...
  public void init(@NonNull Game game) {
    if (this.game == null) {
      ...
      cache = Caffeine.newBuilder().maximumSize(100).expireAfterWrite(Duration.ofMinutes(2))
        .buildAsync(this.game.storage()::createProfile);
    }
  }
  ...
  public void register(@NonNull User user) {
    ...
    if (user instanceof BendingPlayer bendingPlayer) {
      ...
      EventBus.INSTANCE.postPlayerLoadEvent(bendingPlayer);
    } else {
