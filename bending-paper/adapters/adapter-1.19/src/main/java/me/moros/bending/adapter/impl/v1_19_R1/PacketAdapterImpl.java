...
public final class PacketAdapterImpl implements PacketAdapter {
  ...
  private final CompatibilityLayerImpl adapter;

  PacketAdapterImpl(@NonNull CompatibilityLayerImpl adapter) {
  ...
  public void sendNotification(@NonNull Player player, @NonNull Material material, @NonNull Component title) {
    ...
    for (var packet : packets) {
      adapter.player(player).connection.send(packet);
    }
  }
  ...
  public int createFallingBlock(@NonNull World world, @NonNull Vector3d center, @NonNull BlockData data, @NonNull Vector3d velocity, boolean gravity) {
    final int id = Entity.nextEntityId();
    Collection<Packet<?>> packets = new ArrayList<>();
    final int blockDataId = net.minecraft.world.level.block.Block.getId(adapter.blockState(data));
  ...
  private void broadcast(Collection<Packet<?>> packets, World world, Vector3d center, int dist) {
    ...
    for (var player : adapter.world(world).players()) {
  ...
  private ClientboundUpdateAdvancementsPacket createNotification(Material material, Component title) {
    ...
    net.minecraft.world.item.ItemStack icon = adapter.itemStack(material);
  ...
  private ClientboundBlockUpdatePacket fakeBlock(Vector3d b, BlockData data) {
    return new ClientboundBlockUpdatePacket(new BlockPos(b.x(), b.y(), b.z()), adapter.blockState(data));
  }
  ...
  private Collection<Packet<?>> refreshBlocks(Collection<Block> blocks) {
    int size = blocks.size();
    if (size == 0) {
      return List.of();
    } else if (size == 1) {
      var b = blocks.iterator().next();
      return List.of(new ClientboundBlockUpdatePacket(new BlockPos(b.getX(), b.getY(), b.getZ()), adapter.blockState(b)));
    } else {
      Map<SectionPos, Short2ObjectMap<BlockState>> sectionMap = new HashMap<>();
      for (Block b : blocks) {
        BlockState state = adapter.blockState(b);
  ...
  private ClientboundSetEquipmentPacket setupEquipment(int id, Material material) {
    return new ClientboundSetEquipmentPacket(id, List.of(new Pair<>(EquipmentSlot.HEAD, adapter.itemStack(material))));
  }
