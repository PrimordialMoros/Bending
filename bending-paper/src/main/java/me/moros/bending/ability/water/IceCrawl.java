...
import me.moros.bending.ability.common.Line;
...
public class IceCrawl implements Ability {
	...
	public boolean activate(User user, ActivationMethod method) {
		...
		if (method == ActivationMethod.SNEAK && line.isPresent()) {
			...
			if (state instanceof SelectedSource) {
				((SelectedSource) state).attemptRefresh(source.get());
			}
			return false;
		}
	...
	private void launch() {
		...
		if (state instanceof SelectedSource) {
			...
			if (src.isPresent()) {
				iceLine = new IceLine(user, src.get());
	...
	private class IceLine extends Line {
		public IceLine(User user, Block source) {
	...
	public static class Config extends Configurable {
		...
		public void onConfigReload() {
			...
			iceDuration = abilityNode.getNode("ice-duration").getLong(0);
			freezeDuration = abilityNode.getNode("freeze-duration").getLong(0);
